...
int main() {
    ...
    string fileName; 
    cout << "specify a file name (with extension): ";
    cin >> fileName;
    
    ofstream outf;                /* An output file stream. */
    outf.open(fileName.c_str(), ios_base::out | ios_base::in);          /* Creates (or overwrites) a disk file for output. */
    if (outf.is_open()){
        string ans;
        cout << "file is open, proceed? y/n";
        cin >> ans;
        if (ans == "no"){
            outf.close();
        }
